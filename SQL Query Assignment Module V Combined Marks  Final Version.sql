...
SELECT * FROM gradeSummary ORDER BY studentID ASC;
...
SELECT studentID, First_name, Lastname, COUNT(*) occurences
FROM masterModuleV
GROUP BY studentID, First_name, Lastname
HAVING COUNT(*) > 1;
...
FROM masterModuleV
UNION
SELECT studentID, 'Assignment 1', assignment1
FROM masterModuleV
UNION
SELECT studentID, 'Assignment 2', assignment2
FROM masterModuleV;
...
SELECT studentTable.studentID, studentTable.firstName, studentTable.lastName, marksTable.marksMark AS Midterm, marksTable.marksMark AS Final, gradeSummary.marksTotal, gradeSummary.marksAverage, gradeLetters.gradeLetter
FROM studentTable
INNER JOIN gradeSummary
ON studentTable.studentID = gradeSummary.studentID
INNER JOIN gradeLetters
...
WHERE marksTable.marksType = 'midterm'
INNER JOIN marksTable
ON studentTable.studentID = marksTable.studentID
WHERE marksTable.marksType = 'final'
